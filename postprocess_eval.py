...
def get_candidate_tables(format_sql, schema):
    ...
    for ii, token in enumerate(tokens):
        if '.' in token:
            ...
            if table_name in table_names_original:
...
def postprocess_single(format_sql_2, schema, start_alias_id=0):
    ...
    if len(candidate_tables_id) > 1:
        child_col = 0
        child_table = ''
        for idx in candidate_tables_id:
            child = table_names_original[idx]
            column_names = [j for i, j in schema['column_names_original'] if i == idx and j.endswith('Id')]
            if len(column_names) > child_col:
                child_table = child
                child_col = len(column_names)
        candidate_tables_id = [table_names_original.index(child_table)]
...
def postprocess_one(pred_sql, schema):
    pred_sql = pred_sql.replace('group_by', 'group by').replace('order_by', 'order by').replace('limit_value',
                                                                                                'limit 1').replace(
        '_EOS', '').replace(' value ', ' 1 ').strip(',').strip()
...
def write_and_evaluate(postprocess_sqls, db_path, table_schema_path, gold_path, dataset):
    ...
    if dataset == 'spider':
        ...
        command = 'python3 eval_scripts/evaluation_wiki.py --db {} --table {} --etype match --gold {} --pred {}'.format(
            db_path,
            table_schema_path,
            gold_path,
            os.path.abspath(output_file))
        ...
        command = 'python3 eval_scripts/evaluation_soql.py --db {} --table {} --etype match --gold {} --pred {}'.format(
            db_path,
            table_schema_path,
            gold_path,
            os.path.abspath(output_file))
        ...
        command = 'python3 eval_scripts/evaluation_sqa.py --db {} --table {} --etype match --gold {} --pred {}'.format(
            db_path,
            table_schema_path,
            gold_path,
            os.path.abspath(output_file))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'spider':
        ...
        table_schema_path = 'data/wiki/tables.json'
        if args.split == 'dev':
            gold_path = 'data/wiki/dev_gold.txt'
    elif args.dataset == 'soql':
        table_schema_path = 'data/soql/tables.json'
        if args.split == 'dev':
            gold_path = 'data/soql/dev_gold.txt'
    ...
    # for p in postprocess_sqls['salesforce_2']:
    #   print (json.dumps(p, indent=2))
    #   input()
